...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp|ClassExp;//added classexp here
...
export type ClassExp={tag: "ClassExp"; fields:VarDecl[] ;methods:Binding[];}//added classexp here
...
export const makeClassExp=(fields:VarDecl[], methods:Binding[]): ClassExp =>
    ({tag:"ClassExp" , fields:fields,methods:methods});//added ckassexp here
...
op==="if"? parseIfExp(params): op=="lambda"? isNonEmptyList<Sexp>(params)? parseProcExp(first(params),rest(params)):makeFailure(`Bad proc exp: ${params}`): op==="let"? isNonEmptyList<Sexp>(params)? parseLetExp(first(params),rest(params)): makeFailure(`Bad let exp: ${params}`): op=== "quote"? isNonEmptyList<Sexp>(params)?parseLitExp(first(params)):makeFailure(`Bad quote exp: ${params}`):op==="class"? isNonEmptyList<Sexp>(params)? parseClassExp(first(params),second(params)): makeFailure(`Bad class exp:${params}`): makeFailure("never");
...
///// class parsing::
//======================
const parseClassExp =(variables :Sexp,body :Sexp):Result<ClassExp> =>{
    if(isArray(variables) && allT(isString, variables)) {
          if (!isGoodBindings(body)) {
        return makeFailure('Malformed bindings in "let" expression');
    }
    const fields : VarDecl[] = map(makeVarDecl, variables);
    const vars = map(b => b[0], body);
    const valsResult = mapResult(parseL3CExp, map(second, body));
    const methods : Result<Binding[]> = bind(valsResult, (vs: CExp[]) => makeOk(zipWith(makeBinding, vars, vs)));
    return bind(methods, (ms : Binding[]) => makeOk(makeClassExp(fields,ms)));}
     else
    return makeFailure(`Invalid vars for ProcExp ${format(variables)}`);
}



/////////////////end of class parsing
const parseProcExp = (vars: Sexp, 
    body: Sexp[]): Result<ProcExp> =>
...
import { isNone } from "../shared/optional";
