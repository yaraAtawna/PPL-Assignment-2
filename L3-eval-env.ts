...
const applyClass = (classVal: ClassValue, args: Value[]): Result<Value> => 
    !isNonEmptyList<Value>(args) 
        ? makeFailure('cant create object without the fields') 
        : makeOk(makeObjectEnv(
            classVal.methods, 
            makeExtEnv(
                classVal.fields.map((f: VarDecl) => f.var), 
                args, 
                classVal.env
            )
        ));
...
const applyObject = (obj: Object, args: Value[]): Result<Value> => {
    if (!isNonEmptyList<Value>(args)) {
        return makeFailure("No provided method name....!");
    }

    if (!isSymbolSExp(args[0])) {
        return makeFailure("First argument isn't a method name");
    }
    
    const name = args[0].val;
    const names = obj.methods.map((b: Binding) => b.var.var);

    if (!names.includes(name)) {
        return makeFailure(`Unrecognized method: ${name}`);
    }

    const method = obj.methods[names.indexOf(name)];
    const methodArgs = args.slice(1);

    return bind(applicativeEval(method.val, obj.env), (v: Value) => 
        applyProcedure(v, methodArgs)
    );
}
